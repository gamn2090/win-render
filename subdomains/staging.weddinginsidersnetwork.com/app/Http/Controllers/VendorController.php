...
class VendorController extends Controller
{
    ...
    public function vendorDashboard(Request $request){
        ...
        $data = [
            ...
            "recentConversations" => $vendor->recentConversations(3),
        ...
        return view('vendor_dashboard', ['data' => $data, 'page' => 'dashboard']);
    }
    ...
    public function vendorList(Request $request){
        ...
        $data["vendors"] = $request->user()->connections()->get();
    ...
    public function clientList(Request $request){
        ...
        $pairs = Pairing::where('vendor_id', $request->user()->id)->pluck('client_id')->toArray();
        $data["clients"] = User::whereIn('id', $pairs)->get();
        return view('vendor.client_list', ['data' => $data, 'page' => 'client_list']);
    }
    ...
    public function findCouples(Request $request){
        ...
        })->where('created_at', '<=', Carbon::now()->subHours(24))->with('pairing', 'inquiries')->orderBy('created_at', 'desc');
        if($request->wedding_date == 'false'){
            $searchResults = $searchResults->where('wedding_date', '!=', null);
        }
        $data["clients"] = $searchResults->paginate(10);
    ...
    public function inbox(Request $request){
        ...
        $data["conversations"] = $request->user()->getAllConversations();
        return view('chat.inbox', ['data' => $data, 'page' => 'inbox']);
    }
    ...
    public function removePairing(Request $request){
        $client_id = $this->clientService->uuidToID($request->client_uuid);
        if(!$client_id){
            return ["status" => false, "message" => "Client not found"];
        }
        $pairing = $request->user()->pairingWith($client_id);
        if(!$pairing){
