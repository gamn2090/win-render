...
{    
   ...
   protected $fillable = [
    ...
    public function pairing(){
        return $this->hasMany(Pairing::class, 'vendor_id');
    }

    public function pairingWith($clientId){
        return $this->pairing->where('client_id', $clientId)->first();
    }
    ...
    */
    ...
    public function updateAllRankingScores(){
        ...
        $communityScore = ($communityValue["value"] / $communityValue["max"]) * 100;
        $vendorCommunityValue = $this->vendorCommunityRankValue();
        $vendorCommunityScore = ((($vendorCommunityValue["value"] / $vendorCommunityValue["max"]) * 100) + (($this->quarterlyReferrals() / 3) * 100))/2;
        $reviewsScore = ($this->googleRating() / 5) * 100;
        $endorsementsScore = ($this->endorsementsScore() / 4) * 100;
        $badgesScore = (count(json_decode($this->badges, true)) / 4) * 100;
        $rankingModel = VendorRanking::updateOrCreate(
            ['vendor_id' => $this->id],
            ['client_community' => $communityScore, 'vendor_community' => $vendorCommunityScore, 'reviews' => $reviewsScore, 'endorsements' => $endorsementsScore, 'badges' => $badgesScore]
        );
    ...
    public function googleRating(){
        return $this->profile->google_review_score;
    }
