<div class="max-w-sm rounded overflow-y-visible mb-2 flex flex-col">
    ...
    <div class="px-6 pt-auto w-full text-center self-end text-bottom pb-6 bg-win-light rounded-b-2xl">
        ...
        @endif
        <a href="/message/vendor/{{ $vendor->uuid }}" class="button-text uppercase mt-2 py-2 w-[90%] md:w-[75%] inline-flex justify-center items-center rounded-lg bg-win-orange text-white disabled:opacity-50 disabled:pointer-events-none hover:cursor-pointer">
            Message
        </button>
        <a class="button-text uppercase mt-2 py-2 w-[90%] md:w-[75%] inline-flex justify-center items-center rounded-lg bg-win-purple text-white disabled:opacity-50 disabled:pointer-events-none" href="/vendor/profile/{{ $vendor->uuid }}">
            View Storefront
        </a>
    </div>
</div>